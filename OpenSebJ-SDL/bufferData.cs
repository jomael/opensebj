...
{
	/// <summary>
	/// Summary description for bufferData.
	/// </summary>
	public class bufferData
	{
		...
		public byte[] getSampleData(int sample, bool leftChannel, int position)
		{
			numOfBytes = dsInterface.aSound[sample].Caps.BufferBytes;
			int bytesPerSample = (dsInterface.aSound[sample].Format.BitsPerSample * dsInterface.aSound[sample].Format.Channels) / 8;

			CreateStreamBuffers();
			CreateStreams();

			// The level of the sample
			byte[] Points = new byte[numOfBytes];
			
			// Read the complete stream in to a memory stream
			dsInterface.aSound[sample].Read(0,stream0,numOfBytes,Microsoft.DirectX.DirectSound.LockFlag.EntireBuffer);	

			// Get the data from the sample
			//for(int i=0; i < numOfBytes - bytesPerSample; i = i + bytesPerSample)
			for(int i=0; i < numOfBytes; i++)
			{
				Points[i] = streamBuffer0[i];				
			}
