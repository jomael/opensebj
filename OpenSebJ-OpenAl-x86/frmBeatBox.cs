...
namespace OpenSebJ
{
	...
	public class frmBeatBox : System.Windows.Forms.Form
    {
        ...
        int DISPLAYED_ROWS = 5;

        private CheckBox[][] bBoxList;
        private Label[] lblBBoxList;
        ...
        private void ReCheckBeats()
        {
            for (int i = 0; i < this.DISPLAYED_ROWS; i++)
            {
                for (int k = 0; k < this.MAX_COLS; k++)
                {
                    if (beatBox.getBeatStat(i + scrollPosition, k) == true)
                    {
                        bBoxList[i][k].BackColor = Color.Red;
                        bBoxList[i][k].Text = "On";
                        bBoxList[i][k].Checked = true;
                    }
                    else 
                    {
                        bBoxList[i][k].BackColor = this.BackColor;
                        bBoxList[i][k].Text = "Off";
                        bBoxList[i][k].Checked = false;
                    }
                }
            }
        }
        ...
        public void redrawMarks()
        {
            ...
            for (int i = 0; i < 32; i++)
            {
                p5 = new Point(93 + (i * (6 + 28)), 280);
                p6 = new Point(93 + (i * (6 + 28)), 290);
                g.DrawLine(timeMarkers, p5, p6);
            }
            ...
            if (playing == true)
            {
                ...
                // thickness of 26. Surprisingly enough the same as the back marks ;-)
                Pen myPen = new Pen(myBrush, 26);
                Point p1 = new Point(93 + (playPos * (6 + 28)), 280);
                Point p2 = new Point(93 + (playPos * (6 + 28)), 290);
        ...
        private void InitChecboxList()
        {
            this.bBoxList = new CheckBox[this.DISPLAYED_ROWS][];

            for (int i = 0; i < this.DISPLAYED_ROWS; i++)
            {
                this.bBoxList[i] = new CheckBox[this.MAX_COLS];
                for (int k = 0; k < this.MAX_COLS; k++)
                {
                    this.bBoxList[i][k] = new CheckBox();
                    this.bBoxList[i][k].Appearance = System.Windows.Forms.Appearance.Button;
                    this.bBoxList[i][k].Location = new System.Drawing.Point(80 + k * 34, 70 + i * 40);
                    this.bBoxList[i][k].Name = string.Format("bBox_{0}_{1}", i, k);
                    this.bBoxList[i][k].Size = new System.Drawing.Size(28, 29);
                    this.bBoxList[i][k].TabIndex = 3 + k; //TODO
                    this.bBoxList[i][k].Text = "Off";
                    this.bBoxList[i][k].CheckedChanged += new System.EventHandler(this.bBox_CheckedChanged);

                    this.Controls.Add(this.bBoxList[i][k]);
                }
            }
        }
        ...
        private void bBox_CheckedChanged(object sender, EventArgs e)
        {
            ...
            string[] words = cb.Name.Split('_');
            
            int selectedRow = Int32.Parse(words[1]);
            int selectedCol = Int32.Parse(words[2]);

            if (bBoxList[selectedRow][selectedCol].Checked)
            {
                bBoxList[selectedRow][selectedCol].BackColor = Color.Red;
                bBoxList[selectedRow][selectedCol].Text = "On";

                beatBox.setBeatOn(selectedRow + scrollPosition, selectedCol);
            }
            else
            {
                bBoxList[selectedRow][selectedCol].BackColor = this.BackColor;
                bBoxList[selectedRow][selectedCol].Text = "Off";

                beatBox.setBeatOff(selectedRow + scrollPosition, selectedCol);

            }
        }

        private void InitLabelList(bool isLoaded)
        {
            if (!isLoaded)
            {
                this.lblBBoxList = new Label[this.DISPLAYED_ROWS];

                for (int i = 0; i < this.DISPLAYED_ROWS; i++)
                {
                    this.lblBBoxList[i] = new Label();
                    this.lblBBoxList[i].AutoSize = true;
                    this.lblBBoxList[i].Location = new System.Drawing.Point(55, 70 + i * 40);
                    this.lblBBoxList[i].Name = string.Format("lblBBox_{0}", i);
                    this.lblBBoxList[i].Size = new System.Drawing.Size(13, 13);
                    this.lblBBoxList[i].TabIndex = 19;    //TODO            
                    this.lblBBoxList[i].Text = (i + scrollPosition + 1).ToString();

                    this.Controls.Add(this.lblBBoxList[i]);
                }
            }
            else
            {
        ...
        {
            Label[] colLabels = new Label[this.MAX_COLS];

            for (int i = 0; i < this.MAX_COLS; i++)
            {
                colLabels[i] = new Label();
                colLabels[i].AutoSize = true;
                colLabels[i].Location = new System.Drawing.Point(85 + i * 34, 50);
                colLabels[i].Name = string.Format("lblCol_{0}", i);
                colLabels[i].Size = new System.Drawing.Size(13, 13);
                colLabels[i].TabIndex = 19;    //TODO            
                colLabels[i].Text = (i + 1).ToString();

                this.Controls.Add(colLabels[i]);
            }
        }
    }
}
