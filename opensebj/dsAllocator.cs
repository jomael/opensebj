...
namespace DirectShowLib
{
    ...
    public class Allocator : IVMRSurfaceAllocator9, IVMRImagePresenter9, IDisposable
    {
        ...
        public Allocator(Control control)
        {
            ...
            // Need to get the render surface from the device which has been created
            renderTarget = OpenSebJ.eRender.GetSurface();
        }
        ...
        #region Members of the IVMRSurfaceAllocator9
        ...
        public int StartPresenting(IntPtr dwUserID)
        {
            lock(this)
            {
                if (OpenSebJ.eRender.device == null)
                    return E_FAIL;
        ...
        public int PresentImage(IntPtr dwUserID, ref VMR9PresentationInfo lpPresInfo)
        {
            ...
            //This is called everytime a new frame needs to be drawn!
        ...
        {
            // This redraws the scence 

            int hr = 0;

            try 
            {
                OpenSebJ.eRender.device.SetRenderTarget(0, renderTarget);
                ...
                if (privateTexture != null)
                {
                    ...
                    {
                        OpenSebJ.eRender.device.StretchRectangle(
                            surface,
                    ...
                    Texture frame = privateTexture;
                    OpenSebJ.eRender.renderBackground(frame);
                    if (hr < 0)
                        return hr;
                }
                else
                {
                    if (textures.ContainsKey(lpPresInfo.lpSurf))
                    {
                        ...
                        //eventRender.eRender.renderBackground(textures[lpPresInfo.lpSurf] as Texture);
                        Texture frame = textures[lpPresInfo.lpSurf] as Texture;
                        OpenSebJ.eRender.renderBackground(frame);
                        if (hr < 0)
                            return hr;
                    }
                ...
                OpenSebJ.eRender.device.Present();
                return 0;
            }
            catch (DirectXException e)
            {
        ...
        private bool NeedToHandleDisplayChange()
        {
            ...
            IntPtr currentMonitor = Manager.GetAdapterMonitor(OpenSebJ.eRender.device.CreationParameters.AdapterOrdinal);
            IntPtr defaultMonitor = Manager.GetAdapterMonitor(OpenSebJ.eRender.adapterInfo.Adapter);
